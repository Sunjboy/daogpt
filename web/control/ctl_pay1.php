...
class ctl_pay {
	...
	public function go() {//跳转支付
		...
		if($type==2){//支付宝支付
			...
			$gourl = 'http://sm.03ky.com/payment/alipay_wap/wappay/pay.php?'.http_build_query($orders);
			...
			if ( strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false ) {//微信内
            	header('Location: https://www.03ky.com/getcode.php?auk=demo3&oid='.$oid);
            	exit;
        	}else{
				...
				$orders = array('WIDout_trade_no'=>$row['oid'],'WIDsubject'=>$row['des'],'WIDtotal_amount'=>$row['money'],'WIDbody'=>$row['des']);
				self::h5pay($orders);
				//die("<script> alert('目前不支持微信外支付，敬请期待...');history.go(-2);<script>");
				mod_order::up_order(array('paytype'=>1,'paytime'=>date('Y-m-d G:i:s',time()))," `oid`='".$oid."'");
				}else{//pc扫码
					//die('pc扫码支付');
					$row = mod_order::get_order($oid);
					$orders = $orders = array('WIDout_trade_no'=>$row['oid'],'WIDsubject'=>$row['des'],'WIDtotal_amount'=>$row['money'],'WIDbody'=>$row['des']);
					$httpdata = http_build_query($orders);
					header('Location: http://sm.03ky.com/payment/Wxpay/example/native.php?'.$httpdata);
					die;
	...
	public function wxjsapi(){
        ...
        if(!empty($code)){
            ...
            $appid = "wxd588577f8b5560ef";
            $secret = "9421b9ade54a8c1a6ef6cb6adc7718b3";
		...
		$row['url'] = "http://sm.03ky.com/?ac=".$ac."&oid=".$oid."&token=".base64_encode(md5($oid));
        ...
        $return = send('http://sm.03ky.com/payment/Wxpay/example/jsapi.php',$data);
    ...
    public function h5pay($row){//$row['money']
        $orders = $row;
		$httpdata = http_build_query($orders);
     	header('Location: http://sm.03ky.com/payment/Wxpay/example/h5api.php?'.$httpdata);
		die;
    }
	...
	public function scanquery(){
		...
		if($row['status']=='1'){
			$return['status'] = true;
			$ac=mod_order::typetochannel($row['type']);
			$return['url'] = "http://sm.03ky.com/?ac=".$ac."&oid=".$oid."&token=".base64_encode(md5($oid));
		}else{
			$return['status'] = false;
			$return['url'] = '';
		}
	...
	public function notify() {
        ...
        if($orders){
            if (1 == 1) {//查看商户号以及订单状态
                ...
                header("Location:http://sm.03ky.com/?ac=".$ac."&oid=".$out_trade_no."&token=".base64_encode(md5($out_trade_no)));
                exit;
            }
        }
	}
