...
class ctl_news
{
    ...
    */
	...
	public function search(){
		...
		if($q=='' || $q=='请输入关键字搜索'){
			die("<script> alert('请输入关键字');parent.location.href='http://bazi.xingyunyun.cn/';</script>");
		}
		...
		if (!db::num_rows()){
				die("<script> alert('没有相关内容');parent.location.href='http://bazi.xingyunyun.cn/baike';</script>");
        }else{
				...
				/********* KDT *************/
				$seo_header['title']       = $q.'的命理知识，关于'.$q.'的百科知识-神算网';
				$seo_header['keywords']    = $q;
				$seo_header['description'] = '神算网为你提供【'.$q.'】的命理知识，为您提供最新最全的【'.$q.'】的百科知识【'.$q.'】。百科知识尽在神算网。';
